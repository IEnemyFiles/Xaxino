...
class _CustomSpinWheelState extends State<CustomSpinWheel> with TickerProviderStateMixin {
  ...
  void initState() {
   final controller=  Get.find<RouletteControllers>();
    _defaultLottieController = AnimationController(vsync: this)..duration =  Duration(seconds: controller.roatationDuration);
    _coinsLottieController = AnimationController(vsync: this)..duration =  Duration(seconds: controller.roatationDuration);
    _goldenConfettiLottieController = AnimationController(vsync: this)..duration =  Duration(seconds: controller.roatationDuration);
                              ...
                              items: List.generate(
                                ...
                                (index) => FortuneItem(
                                  child: Container(
                                    ...
                                    padding: const EdgeInsetsDirectional.all(Dimensions.space5),
                                    child: Text(
                                      "${Converter.formatNumber(controller.name[index])} ".replaceAll(".00", "").tr,
                                      textAlign: TextAlign.end,
                                      style: GoogleFonts.crimsonText(color: index.isEven ? const Color(0xffFDEF9A) : const Color(0xff690101), fontWeight: FontWeight.w400),
                                    ),
                                  ), //attention: use custom item
                                  style: FortuneItemStyle(color: Converter.formatNumber(controller.name[index]).replaceAll(".00", "")=="0"?MyColor.greenP: index.isEven ? const Color(0xffCE0005).withOpacity(0.8) : MyColor.colorWhite, borderColor: MyColor.transparentColor, textStyle: regularSmall),
                                ),
                              ),
                              onAnimationEnd: () {
