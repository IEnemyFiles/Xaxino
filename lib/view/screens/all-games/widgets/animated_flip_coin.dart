...
void initState() {
  ...
  controller.animation = Tween(begin: 0.0, end: pi * 20).animate(controller.animationController)
    ..addListener(() {
      setState(() {});
    });
...
Widget _buildCoin() {
  return GetBuilder<HeadTailController>(builder: (controller) {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<HeadTailController>(
      builder: (controller) => Center(
        child: controller.isAnimationShouldRunning ? _buildCoin() : Image.asset(controller.isAdminSelectHead ? MyImages.head : MyImages.tails,  width: 150,
