...
import 'package:xaxino/data/controller/refferal/refferal_screen_controller.dart';
import 'package:xaxino/data/repo/refferal/refferal_screen_repo.dart';
...
class _RefferalScreenState extends State<RefferalScreen> {
  ...
  void initState() {
    ...
    final controller = Get.put(RefferalScreenController(refferalScreenRepo: Get.find()));
    super.initState();
    ...
    return GetBuilder<RefferalScreenController>(
      builder: (controller) => Container(
        ...
        child: Scaffold(
          ...
          body:controller.isLoading ? const CustomLoader() :     Align(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                           ...
                           children: [
                             Expanded(
                               child: SingleChildScrollView(
                                 ...
                                 child: Text("${UrlContainer.domainUrl}?reference=${controller.username!=""?controller.username:controller.savedUsername}",
                             ...
                             InkWell(
                                 onTap: () async{
                                   await Clipboard.setData( ClipboardData(text: "${UrlContainer.domainUrl}?reference=${controller.username}"));
                                   CustomSnackBar.success(successList: [MyStrings.referralLinkCopied]);

                                 },
                                 child: Container(
                                   padding:const EdgeInsets.symmetric(horizontal:Dimensions.space7,vertical: Dimensions.space7),
                                   decoration: BoxDecoration(
                                       color: MyColor.searchFieldColor,
                                       borderRadius: BorderRadius.circular(4)
                                   ),
                                   child: SvgPicture.asset(
                                     ...
                                     colorFilter: const ColorFilter.mode(MyColor.activeIndicatorColor, BlendMode.srcIn),
                                   ),
                                 ))
                           ],
                         ),
                ...
                controller.refferalData.isEmpty?const NoDataTile(title: MyStrings.noRefferanceFound,):  ListView.builder(
