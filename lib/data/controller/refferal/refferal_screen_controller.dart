...
import 'package:xaxino/data/model/refferal/refferalModel.dart';
...
class RefferalScreenController extends GetxController {
  ...
  getUsername() {
    savedUsername = refferalScreenRepo.apiClient.sharedPreferences.getString(SharedPreferenceHelper.userNameKey) ?? "";
    update();
  }
  ...
  void allGamesInfo() async {
    ...
    if (model.statusCode == 200) {
      RefferalDataModel refferalModel = RefferalDataModel.fromJson(jsonDecode(model.responseJson));
      username = refferalModel.data?.referralUsers?.username ?? "";

      refferalData.addAll(refferalModel.data?.referralUsers?.allReferrals ?? []);
      update();
    } else {
