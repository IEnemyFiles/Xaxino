...
import 'package:xaxino/data/model/home_screen/home_screen_model.dart';
...
class HomeController extends GetxController {
  ...
  Future<void> loadData() async {
    ...
    firstName =sharedPreferences.getString(SharedPreferenceHelper.fullNameKey) ?? "";
    homeRepo.apiClient.sharedPreferences.setString(SharedPreferenceHelper.defaultCurrencyKey, defaultCurrencySymbol);
  ...
  void allGamesInfo() async {
    ...
    if (model.statusCode == 200) {
      HomeScreenModel responseModel = HomeScreenModel.fromJson(jsonDecode(model.responseJson));
      if (responseModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        ...
        totalBalance = responseModel.data?.widget?.totalBalance.toString() ?? "0.00";
      }
