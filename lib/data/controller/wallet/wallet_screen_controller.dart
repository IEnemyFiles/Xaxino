...
import 'package:xaxino/data/model/transctions/transaction_response_model.dart';
...
class WalletScreencontroller extends GetxController {
  ...
  WalletScreencontroller({
    required this.walletScreenRepo,
  });
  ...
  void getWalletScreenData() async {
    ...
    if (model.statusCode == 200) {
      TransactionResponseModel walletScreenDataModel = TransactionResponseModel.fromJson(jsonDecode(model.responseJson));

      if (walletScreenDataModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        walletBalance = walletScreenDataModel.data?.totalBalance.toString() ?? "0";
  ...
  loadData() async {
    ...
    defaultCurrency = walletScreenRepo.apiClient.sharedPreferences.getString(SharedPreferenceHelper.defaultCurrencyKey) ?? "";
  }
  ...
  Future<void> loadTransaction() async {
    ...
    ResponseModel responseModel = await walletScreenRepo.getTransactionList(page, type: selectedTrxType.toLowerCase(), remark: selectedRemark.toLowerCase(), searchText: trxSearchText);
