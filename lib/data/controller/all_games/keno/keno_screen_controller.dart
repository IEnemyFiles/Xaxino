...
import 'package:xaxino/data/model/keno_data/keno_data_model.dart';
import 'package:xaxino/data/model/keno_data/keno_number_model.dart';
import 'package:xaxino/data/model/keno_result/keno_result_model.dart';
import 'package:xaxino/data/model/keno_submit_ans/keno_submit_ans_model.dart';
import 'package:xaxino/data/repo/all-games/keno/keno_repo.dart';
...
class KenoScreenController extends GetxController {
  KenoRepo kenoRepo;
  KenoScreenController({
    required this.kenoRepo,
  });
  ...
  List<KenoNumberModel> kenoNumbers = List.generate(80, (index) => KenoNumberModel(number: index + 1, isSelected: false, isFromAdmin: false));
  ...
  void loadGameInfo() async {
    ...
    if (model.statusCode == 200) {
      KenoDataModel responseModel = KenoDataModel.fromJson(jsonDecode(model.responseJson));
  ...
  submitInvestmentRequest() async {
    ...
    List<String> selectedNumbersAsString = selectedNumbers.map((number) => "$number").toList();
    ResponseModel model = await kenoRepo.submitAnswer(amountController.text, selectedNumbersAsString);

    if (model.statusCode == 200) {
      KenoSubmitAnsModel submitAnswer = KenoSubmitAnsModel.fromJson(jsonDecode(model.responseJson));
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      KenoResultModel kenoResultModel = KenoResultModel.fromJson(jsonDecode(model.responseJson));

      if (kenoResultModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        adminSelectedNum = kenoResultModel.data!.result;

        adminSelectedNumbers.addAll(kenoResultModel.data!.result!.map((stringNumber) => int.parse(stringNumber)));
        ...
        Timer(const Duration(seconds: 3), () {
          ...
          if (kenoResultModel.data!.win == "1") {
            ...
            CustomAlertDialog(child: LoseDialog(isShowResult: true, result: adminSelectedNumbers.toString(), adminSelectedNumber: matchedNumbers.length.toString(),
              iskenoResult: true,)).customAlertDialog(Get.context!);
            isWinner == false;
            update();
          }
          resetAll();
        ...
        CustomSnackBar.error(errorList: kenoResultModel.message?.success ?? [MyStrings.somethingWentWrong.tr]);
      }
    } else {
      CustomSnackBar.error(errorList: [model.message]);
    }
  ...
  void checkMatchAndSetFlags() {
    for (var i = 0; i < 80; i++) {
      ...
      if (selectedNumbers.contains(numberModel.number) && adminSelectedNumbers.contains(numberModel.number)) {
