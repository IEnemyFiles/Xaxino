...
import 'package:xaxino/data/model/spin_wheel/spin_wheel_data_model.dart';
import 'package:xaxino/data/model/spin_wheel_result/spin_wheel_result_model.dart';
import 'package:xaxino/data/model/spin_wheel_submit_ans/spin_wheel_submit_ans_model.dart';
import 'package:xaxino/data/repo/all-games/spin_wheel/spin_wheel_repo.dart';
...
class SpinWheelControllers extends GetxController {
  ...
  SpinWheelControllers({
    required this.spinWheelRepo,
  });
...
late AudioPlayer audioController;
  ...
  Future<void> loadGameInfo() async {
    ...
    defaultCurrencySymbol = spinWheelRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
    update();
    ResponseModel model = await spinWheelRepo.loadData();
    if (model.statusCode == 200) {
      SpinWheelDataModel responseModel = SpinWheelDataModel.fromJson(jsonDecode(model.responseJson));
  ...
  submitInvestmentRequest() async {
    ...
    ResponseModel model = await spinWheelRepo.submitAnswer(amountController.text, isBlue ? "blue" : "red");

    if (model.statusCode == 200) {
      SpinWheelSubmitAnsModel submitAnswer = SpinWheelSubmitAnsModel.fromJson(jsonDecode(model.responseJson));
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      SpinWheelResultModel rouletteAnswerModel = SpinWheelResultModel.fromJson(jsonDecode(model.responseJson));
      ...
      if (rouletteAnswerModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        Timer(Duration(seconds: roatationDuration), () {
          if (rouletteAnswerModel.data!.type != MyStrings.danger) {
            CustomAlertDialog(child: WinnerDialog(result: rouletteAnswerModel.data?.result.toString()??"")).customAlertDialog(Get.context!);
          } else {
            CustomAlertDialog(child:  LoseDialog(isShowResult: true,result: rouletteAnswerModel.data?.result.toString()??"")).customAlertDialog(Get.context!);
          }
          isSubmitted = false;
