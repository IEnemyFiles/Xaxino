...
import 'package:xaxino/data/model/mines_cashout/mines_cashout_model.dart';
import 'package:xaxino/data/model/mines_end/mines_end_model.dart';
import 'package:xaxino/data/model/mines_game_info_model/mines_game_info_model.dart';
import 'package:xaxino/data/model/mines_invest/mines_invest_model.dart';
import 'package:xaxino/data/model/mines_model/mines_model.dart';
import 'package:xaxino/data/repo/all-games/mines/mines_repo.dart';
...
class MinesScreenController extends GetxController {
  MinesRepo minesRepo;
  MinesScreenController({
    required this.minesRepo,
  });
  ...
  List<MinesModel> mines = List.generate(25, (index) => MinesModel(imagePath: MyImages.treasureBoxOff, isBombed: false, isJewel: false, isLocked: true, tapped: false, tapCounter: 0));
  ...
  void loadGameInfo() async {
    ...
    if (model.statusCode == 200) {
      MinesGameInfoModel responseModel = MinesGameInfoModel.fromJson(jsonDecode(model.responseJson));
  ...
  submitInvestmentRequest() async {
    ...
    ResponseModel model = await minesRepo.invest(amountController.text, minesController.text);

    if (model.statusCode == 200) {
      MinesInvestModel submitAnswer = MinesInvestModel.fromJson(jsonDecode(model.responseJson));

      if (submitAnswer.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        gameId = submitAnswer.data?.gameLogId.toString() ?? "";
        availableMines = int.tryParse(submitAnswer.data?.availableMine.toString() ?? "0") ?? 0;
  ...
  cashOut() async {
    ...
    if (model.statusCode == 200) {
      MinesCashOutModel cashOutModel = MinesCashOutModel.fromJson(jsonDecode(model.responseJson));
      if (cashOutModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        availAbleBalance = cashOutModel.data?.balance.toString() ?? "0";
        CustomAlertDialog(
            child: const WinnerDialog(
          result: "",
        )).customAlertDialog(Get.context!);
        resetAllSelection();
        CustomSnackBar.success(successList: [cashOutModel.data?.success.toString() ?? ""]);
      } else {
        availAbleBalance = cashOutModel.data?.balance.toString() ?? "0";
        CustomAlertDialog(child: const LoseDialog()).customAlertDialog(Get.context!);
        resetAllSelection();
        CustomSnackBar.error(errorList: cashOutModel.message?.error ?? [MyStrings.somethingWentWrong.tr]);
        update();
      }
    } else {
      CustomSnackBar.error(errorList: [model.message]);
  ...
  gameEnd() async {
    ...
    if (model.statusCode == 200) {
      MinesEndModel minesEndModel = MinesEndModel.fromJson(jsonDecode(model.responseJson));

      if (minesEndModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
