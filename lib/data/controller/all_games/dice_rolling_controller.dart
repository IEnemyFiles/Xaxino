...
import 'package:xaxino/data/model/dice_rolling_data/dice_rolling_data_model.dart';
import 'package:xaxino/data/model/dice_rolling_result/dice_rolling_result_model.dart';
import 'package:xaxino/data/model/dice_rollinng_submit_ans/dice_rolling_submit_ans_repo.dart';
import 'package:xaxino/data/model/global/response_model/response_model.dart';
import 'package:xaxino/data/repo/all-games/dice_rolling/dice_rolling_repo.dart';
...
class DiceRollingController extends GetxController {
  ...
  DiceRollingController({
    required this.diceRollingRepo,
  });
  ...
  void loadGameInfo() async {
    ...
    defaultCurrencySymbol = diceRollingRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
    update();
    ResponseModel model = await diceRollingRepo.loadData();
    if (model.statusCode == 200) {
      DiceRollingDataModel responseModel = DiceRollingDataModel.fromJson(jsonDecode(model.responseJson));
  ...
  submitInvestmentRequest() async {
    ...
    if (model.statusCode == 200) {
      DiceRollingSubmitAnsModel submitAnswer = DiceRollingSubmitAnsModel.fromJson(jsonDecode(model.responseJson));

      if (submitAnswer.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        gameId = submitAnswer.data?.gameLog?.id.toString() ?? "0";
        desiredNumber = int.tryParse(submitAnswer.data?.gameLog?.result.toString() ?? "0")!;
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      DiceRollingResultModel rouletteAnswerModel = DiceRollingResultModel.fromJson(jsonDecode(model.responseJson));
