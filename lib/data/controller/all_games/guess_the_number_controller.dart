...
import 'package:xaxino/data/model/guess_the_number_data/guess_the_number_model.dart';
import 'package:xaxino/data/model/guess_the_number_result/guess_the_number_result.dart';
import 'package:xaxino/data/model/guess_the_number_submit_ans/guess_the_number_submit_ans.dart';
...
class GuessTheNumberController extends GetxController {
  ...
  GuessTheNumberController({
    required this.guessTheNumberRepo,
  });
  ...
  void loadGameInfo() async {
    ...
    defaultCurrencySymbol = guessTheNumberRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
    update();
    ResponseModel model = await guessTheNumberRepo.loadData();
    if (model.statusCode == 200) {
      GuessDataModel responseModel = GuessDataModel.fromJson(jsonDecode(model.responseJson));
      ...
      if (responseModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        ...
        guessTheNumberRepo.apiClient.sharedPreferences.setString(SharedPreferenceHelper.savedScreenMsgKey, screenmsg);
  ...
  submitInvestmentRequest() async {
    ...
    ResponseModel model = await guessTheNumberRepo.submitAnswer(amountController.text, guessNumberController.text);
    if (model.statusCode == 200) {
      GuessTheNumberSubmitAnsModel submitAnswer = GuessTheNumberSubmitAnsModel.fromJson(jsonDecode(model.responseJson));
...
String extractNumber(String input) {
  ...
  endTheGame() async {
    ...
    ResponseModel model = await guessTheNumberRepo.getAnswer(gameId, guessNumberController.text);

    if (model.statusCode == 200) {
      GuessTheNumberResultModel guessTheNumberResultModel = GuessTheNumberResultModel.fromJson(jsonDecode(model.responseJson));

      updateWinningPercentage();

      if (guessTheNumberResultModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        screenmsg = guessTheNumberResultModel.data!.message.toString();

        type = guessTheNumberResultModel.data?.type.toString() ?? "";
        if (checkCounter > int.tryParse(chances)! || guessTheNumberResultModel.data!.winStatus == "1") {
          if (guessTheNumberResultModel.data!.winStatus == "1") {
            CustomAlertDialog(child: WinnerDialog(result: guessTheNumberResultModel.data!.winNumber.toString())).customAlertDialog(Get.context!);
            resetAll();
          }
        ...
        if (guessTheNumberResultModel.data!.winStatus == "0") {
          ...
          Timer(const Duration(seconds: 3), () {
            ...
            CustomAlertDialog(child:  LoseDialog(isShowResult: true,result: number.toString())).customAlertDialog(Get.context!);
            resetAll();
          });
          availAbleBalance = guessTheNumberResultModel.data?.bal ?? "";
        }
  ...
  resetAll() {
    ...
    updateWinningPercentage();
    countGuessingTime = 0;
    screenmsg = guessTheNumberRepo.apiClient.sharedPreferences.getString(SharedPreferenceHelper.savedScreenMsgKey) ?? "";
    update();
  }
}
