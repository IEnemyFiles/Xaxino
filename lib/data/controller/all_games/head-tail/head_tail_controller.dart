...
import 'package:xaxino/data/model/head_tail_ans/head_tail_ans_model.dart';
import 'package:xaxino/data/model/head_tail_submit_ans/head_tail_submit_ans_model.dart';
import 'package:xaxino/data/repo/all-games/head-tail/head_tail_repo.dart';
...
class HeadTailController extends GetxController {
  ...
  void loadData() async {
    ...
    defaultCurrencySymbol = headTailRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
  ...
  submitInvestmentRequest() async {
    ...
    update();
    ResponseModel model = await headTailRepo.submitAnswer(amountController.text, isUserChoiseIsHead! ? "head" : "tail");

    if (model.statusCode == 200) {
      HeadTailInvestmentResponseModel submitAnswer = HeadTailInvestmentResponseModel.fromJson(jsonDecode(model.responseJson));

      if (submitAnswer.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        gameId = submitAnswer.data?.gameLog?.id.toString() ?? "";
        availableBalance = Converter.formatNumber(submitAnswer.data?.balance ?? "0");

        isAdminSelectHead = submitAnswer.data!.gameLog?.result == "head" ? true : false;
        startGame();
        update();
      } else {
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      HeadTailAnsModel headTailAnsModel = HeadTailAnsModel.fromJson(jsonDecode(model.responseJson));

      if (headTailAnsModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        isAdminSelectHead = headTailAnsModel.data!.result == "head" ? true : false;

        stopFlip();

        Timer(const Duration(seconds: 3), () {
          if (headTailAnsModel.data != null && headTailAnsModel.data!.type != MyStrings.danger) {
            CustomAlertDialog(
                child: WinnerDialog(
