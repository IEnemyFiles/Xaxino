...
import 'package:xaxino/data/model/card_finding_result/card_finding_result.dart';
import 'package:xaxino/data/model/card_finding_submit_ans/card_finding_submit_ans_model.dart';
...
import 'package:xaxino/data/repo/all-games/card_finding/card_finding_repo.dart';
...
class CardFindingController extends GetxController {
  ...
  CardFindingController({
    required this.cardFindingRepo,
  });
  ...
  submitInvestmentRequest() async {
    ...
    if (model.statusCode == 200) {
      CardFindingSubmitAnsModel submitAnswer = CardFindingSubmitAnsModel.fromJson(jsonDecode(model.responseJson));
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      CardFindingResultModel cardFindingResultModel = CardFindingResultModel.fromJson(jsonDecode(model.responseJson));
      ...
      if (cardFindingResultModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        ...
        Timer(const Duration(seconds: 10), () {
          if (cardFindingResultModel.data?.type != MyStrings.danger) {
            CustomAlertDialog(child: WinnerDialog(result: cardFindingResultModel.data?.result.toString() ?? "")).customAlertDialog(Get.context!);
          } else {
            CustomAlertDialog(child:  LoseDialog(isShowResult: true,result: cardFindingResultModel.data?.result.toString() ?? "")).customAlertDialog(Get.context!);
          }
  ...
  Future<void> loadCurrency() async {
    ...
    defaultCurrencySymbol = cardFindingRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
  }
