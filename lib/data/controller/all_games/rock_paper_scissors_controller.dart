...
import 'package:xaxino/data/model/rock_paper_scissors/rock_paper_scissors_model.dart';
import 'package:xaxino/data/model/rock_paper_scissors_ans/rock_paper_answer_model.dart';
import 'package:xaxino/data/model/rock_paper_scissors_submit_ans/rock_paper_scissors_submit_answer_model.dart';
import 'package:xaxino/data/repo/all-games/rock_paper_scissors/rock_paper_scissors_repo.dart';
...
class RockPaperScissorsController extends GetxController {
  ...
  final List<String> rockScissorPaperImages = [MyImages.rock, MyImages.bigPaper, MyImages.bigScissors];
  ...
  void changeRockScissorsPapersImages() {
    ...
    currentIndex = (currentIndex + 1) % rockScissorPaperImages.length; // 1 % 3 = 1
    update();
  }
  ...
  void initData() async {
    ...
    defaultCurrencySymbol = rockPaperScissorsRepo.apiClient.getCurrencyOrUsername(isSymbol: true);
    update();
  ...
  Future<void> loadGameInfo() async {
    ...
    if (model.statusCode == 200) {
      RockpaperScissorsDataModel responseModel = RockpaperScissorsDataModel.fromJson(jsonDecode(model.responseJson));
  ...
  submitInvestmentRequest() async {
    ...
    ResponseModel model = await rockPaperScissorsRepo.submitAnswer(amountController.text, choice);

    if (model.statusCode == 200) {
      RockpaperScissorsSubmitAnswerModel submitAnswer = RockpaperScissorsSubmitAnswerModel.fromJson(jsonDecode(model.responseJson));
  ...
  endTheGame() async {
    ...
    if (model.statusCode == 200) {
      RockpaperScissorsAnswerModel headTailAnsModel = RockpaperScissorsAnswerModel.fromJson(jsonDecode(model.responseJson));

      if (headTailAnsModel.status?.toLowerCase() == MyStrings.success.toLowerCase()) {
        ...
        Timer(const Duration(seconds: 8), () {
          ...
          if (headTailAnsModel.data?.type != MyStrings.danger) {
            CustomAlertDialog(child: WinnerDialog(result: headTailAnsModel.data?.result.toString() ?? "0")).customAlertDialog(Get.context!);

            update();
          } else {
            CustomAlertDialog(child: LoseDialog(isShowResult: true, result: headTailAnsModel.data?.result.toString() ?? '')).customAlertDialog(Get.context!);
          }
